...
5. `new-article.md` 默认使用 `weekly-writing` 作为输出练习，不需要再填写名称；如果尚未立项，首次使用时由 AI 在 `practices/` 中建立最小结构。
...
- 日记记录和笔记默认直接保存；文章工作台只发展已经选定的主题，未确定主题时请使用者先提供或确认，不在文章工作台中调用 `weekly-review` 替其选题。
